...
```
Brief ─▶ ⓪ Trend ▶ ① Discovery ▶ ② Curation ▶ ③ Copy ▶ ④ Compliance ▶ ⑤ Media(선택) ▶ ⑥ Publish
          키워드       발굴           선별★        카피        검수           소재 기획        게시 준비
            ▲                          ▲
            └── (니치·시즌)            └── 카테고리 부스트 ◀── ⑦ Insight (성과→학습 루프)
```
...
**역할**: 키워드 전략가. "뭘 검색할지"를 스스로 정한다 — 파이프라인에서 유일하게
사람이 매번 생각해야 했던 입력(keyword)을 자동화.

| | |
|--|--|
| 입력 | `Brief`(niche·audience·market), `AgentLog` |
| 출력 | `list[str]` 키워드 후보 (터질 가능성 순) |
| Claude | 선택 — 시즌·트렌드 반영 생성. mock=니치·시즌 시드 + **날짜 로테이션**(매일 다른 후보) |

`brief.keyword`/`category_id` 가 없을 때만 오케스트레이터가 실행해 1순위 키워드를
채운다. `--niche 건강`(ko) / `--niche kitchen`(en) 으로 힌트를 줄 수 있고, 없으면
전체 풀에서 뽑는다. 시즌(월별: 여름=쿨링/cooling)이 마지막 후보에 반영된다.
...
`run(..., boosts=)` 로 InsightAgent 의 **성과 학습 부스트**(수익 난 카테고리 가점)를
받는다 — 오케스트레이터가 주입.
...
**역할**: 검수 문지기. 효능 단정·과장 표현을 게시 전에 걸러 **계정 정지·법적
리스크**(한국 표시광고법·의료기기법, 미국 FTC)를 차단한다. 40+ 타깃은 건강 제품
비중이 높아 특히 중요.

| | |
|--|--|
| 입력 | `Brief`, `list[Caption]`, `AgentLog` |
| 출력 | (순화된 `list[Caption]`, `ComplianceReport`) |
| Claude | 선택(순화 문장 다듬기). **규칙 기반은 무료로 항상 실행** |

- 언어별 규칙(`_RULES_KO`/`_RULES_EN`): "치료→관리", "cure→help with" 등 자동 순화.
- 자동 순화 불가 건("의사 추천", "FDA-approved")은 `report.flagged` 로 남겨
  **게시 전 수동 확인**을 요구한다 (로그에 ⚠ 표시).
- 사슬 위치: **Copy 다음, Media/Publish 앞** — 순화된 카피가 TTS 대본·렌더본에
  반영되도록.
...
## ⑦ InsightAgent — `insight.py` (피드백 루프)

**역할**: 성과 분석가. 쌓인 클릭·주문·수수료를 읽어 "뭐가 터졌는지"를 찾고,
그 학습을 다음 선별에 되먹인다 — 던지고→배우는 루프의 클로저.

| | |
|--|--|
| 입력 | `Storage`, `AgentLog` |
| 출력 | `InsightReport` (카테고리·플랫폼 집계, 추천, `category_boosts`) |
| Claude | 없음 — 순수 집계 |

- **소표본 보호**: 총 클릭 `MIN_TOTAL_CLICKS`(30) 미만이면 부스트를 내지 않는다
  (노이즈 과적합 방지).
- 부스트 = 카테고리별 수수료 점유율 × `MAX_BOOST`(40) — 40+ 가점과 같은 스케일.
- 오케스트레이터가 `curate()` 때마다 `category_boosts()` 를 조회해 Curation 에 주입.
- CLI: `python -m kupas insights` — 사람이 읽는 리포트(집계·추천·부스트).
...
- `curate(brief)`: (keyword 없으면 Trend) → 발굴 → 선별(+Insight 부스트). 무료 triage
- `run(brief)`: 단일 마켓 전체 사슬 — curate → Copy → Compliance → (Media) → Publish
  (+`save` 시 SQLite 저장)
