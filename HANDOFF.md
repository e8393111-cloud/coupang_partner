...
⚠️ **이 우회는 「첫 발행」 시점이 전부다.** 주소는 발행 버튼을 누르는 순간의 제목으로
만들어지고 그 뒤엔 못 고친다. 제목이 한글이면 슬러그 변환이 안 돼 `blog-page`,
`blog-page_04`, `blog-page_857` 처럼 떨어진다 — 오늘만(`oneulmandeal`) 3종이 실제로 그렇게 됐다.
발행 **전에** 제목을 한글로 바꾸면 우회가 무효다. 발행 직후 주소부터 확인할 것.
