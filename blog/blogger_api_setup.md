...
python3 tools/publish_blogger.py backup --blog oneulmandeal.blogspot.com
```

`blog/backup/<블로그>/` 에 글·페이지 본문이 받아집니다. 초안도 포함됩니다.

**블로거 자체 백업과 목적이 다릅니다. 둘 다 하세요.**

| | 블로거 XML 백업 | 이 명령 |
|---|---|---|
| 위치 | 받은 기기 | 깃허브 (블로그와 함께 사라지지 않음) |
| 형식 | XML (복원용) | HTML (읽기·재사용) |
| 복원 | 다른 블로그로 그대로 | 손으로 다시 올림 |
| 이력 | 없음 | git 이력 |

블로거 XML 은 `설정 > 관리 > 콘텐츠 백업`, **테마는 별도**로 `테마 > ⋮ > 백업`.

⚠️ **이미지는 어느 쪽에도 안 들어갑니다.** 블로거 이미지는 구글 포토에 따로
저장되고 본문에는 주소만 남습니다. 블로그가 삭제되면 이미지도 같이 사라질 수 있으니,
글에 쓴 캡처는 원본을 따로 보관하세요.
