...
> 🔴 **어드민 화면이 CIDR 을 거부합니다 — 문서와 다릅니다.**
> `160.79.104.0/21` 을 넣으면 "올바른 IP 주소 형식이 아니에요" 가 뜹니다 (2026-08-05 실측).
> 그런데 공식 문서 `guide/open-api/auth.md` 는 **"단일 IP 또는 CIDR 대역
> (예: `203.0.113.10`, `203.0.113.0/24`)"** 이라고 명시합니다. 문서 챗봇에 다시 물어도
> "CIDR 대역 등록은 실제로 지원됩니다" 라고 답합니다. **화면 쪽 버그로 보인다.**
>
> → 토스에 문의해 열리면 **서버가 필요 없어진다.** 아래 참조.
> **문의 발송 2026-08-05** (초안 `blog/toss_cidr_inquiry.txt`). 답변 대기 중.
