...
사업자당 **10개**까지, **단일 IP 만** 등록됩니다.

> 🔴 **CIDR 대역은 안 됩니다.** 문서에는 `203.0.113.0/24` 같은 대역도 된다고 적혀 있지만
> 어드민 화면이 "올바른 IP 주소 형식이 아니에요" 로 거부합니다 (2026-08-05 실측).
> **문서보다 화면이 진실입니다.**

> 🔴 **이 세션(클라우드) 환경에서는 API 를 호출할 수 없습니다.**
> 출발지 IP 가 `160.79.106.0/24` 대역에서 **호출마다 새로 뽑힙니다.**
> 한 프로세스 안에서는 고정이지만 프로세스가 바뀌면 달라집니다 —
> 6번 뽑아 `.4 .21 .66 .100 .104 .124` 로 전부 달랐습니다.
> 254개 중 10개만 등록할 수 있으니 덮을 수가 없고, 프록시에 IP 를 고정하는 옵션도 없습니다.
> **→ 고정 IP 가 있는 곳(집 PC·공유기·VPS)에서 실행해야 합니다.**

집 인터넷은 보통 며칠~몇 주 같은 IP 를 씁니다. 바뀌면 `whoami` 로 다시 확인해 슬롯을 갈면 됩니다.
